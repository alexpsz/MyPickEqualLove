...
MyPickEqualLove 是面向 ＝LOVE 粉丝的非官方静态 Web App。用户可以从当前歌曲目录中选择最多 10 首喜爱歌曲，形成个人 Top 10 pick board，并生成适合社交平台分享的图片。

应用运行在浏览器端，没有后端账号系统、数据库或服务端 API。歌曲与成员数据来自项目内静态 JSON，经 `src/data/songs.ts` 派生为运行时集合；用户选择与导出选项保存在浏览器 `localStorage`。

本项目为非官方粉丝项目。＝LOVE 名称、歌曲标题、图片和相关素材权利归原权利方所有。

## 2. 目标用户

目标用户是希望快速制作 ＝LOVE 喜爱歌曲分享图的粉丝。

用户应能在不登录、不配置账号、不理解项目技术细节的情况下完成选择、搜索、筛选、预览、下载和分享到 X。

## 3. 核心价值

应用的核心价值是把「从 ＝LOVE 歌曲目录中选择 Top 10」压缩成一个轻量、可分享、可重复编辑的浏览器体验。
...
导出画布由隐藏的离屏 `ExportBoard` 提供，元素 id 为 `mypick-equal-love-export-canvas`。
...
默认无搜索词且未打开已毕业成员显示时，包含已毕业成员参与的歌曲会被隐藏。打开 Graduated 后会显示并允许筛选已毕业成员。

## 9. 数据与内容规则

歌曲数据来自 `src/data/equal-love-songs.json`，成员数据来自 `src/data/equal-love-members.json`。

`src/data/songs.ts` 对歌曲按 releaseDate 降序、标题罗马音升序排序，并派生运行时索引和筛选集合。

歌曲封面使用本地 `public/covers/equal-love/` 下的静态资源，以保证静态导出和图片生成时可用。

数据变更后必须运行 `npm run validate:data`，确保歌曲数量、成员数量、字段枚举、封面文件、成员引用、credits 和边界歌曲规则仍然有效。
