...
应用现在支持配置驱动的 Live Pick 特别页。普通版根路径 `/` 继续是 Top 10；当前 `equal-love` 站点新增 `/live/kokuritsu-2026/` 国立余韵五格页和 `/live/tokyo-dome-2027/` 东京巨蛋愿望五格页。
...
- ＝LOVE 国立余韵五格页：支持 `DAY 1 / DAY 2 / 2 DAYS`，前四格限制为对应 setlist，第五格允许全曲库。
- ＝LOVE 东京巨蛋愿望五格页：五格全部允许从全曲库选择，不区分 Day 1 / Day 2。
...
在国立余韵页中，用户可以选择 `DAY 1`、`DAY 2` 或 `2 DAYS`。三种 context 的 picks 分开保存；切换 context 不会删除其他 context 的选择。用户从前四格打开搜索时只看到当前 context 实际披露曲；从第五格或全局搜索可看到全曲库，但非 setlist 歌曲只能进入第五格。
...
普通版导出图继续使用 2×5 Top 10 grid。Live 五格页使用纵向 memory list，必须显示 event title、context、槽位标题、歌曲封面、昵称和当前特别页路径。
...
国立页前四格不能选入当前 context 未披露歌曲，第五格可以选择全曲库。全局搜索和替换弹窗都必须遵守该规则。

国立页导出图显示 event、context、五格标题、昵称和 `/live/kokuritsu-2026/` 页面路径；X 分享使用国立页面 URL 和专属 hashtag。

东京巨蛋页 `/live/tokyo-dome-2027/` 可访问，显示五个愿望槽位，不出现 Day selector，五格都能从全曲库选择。
