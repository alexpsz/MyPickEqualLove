...
- 下载导出图片。
- 分享文案到 X。
- 通过左上角 `Other Picks` 抽屉跳转到另外两个姐妹 MyPick 站点。

## 5. 用户流程

用户进入首页后看到品牌头部、控制区和 10 个 pick 槽位。

用户可以点击左上角菜单按钮打开 `Other Picks` 抽屉，访问另外两个姐妹 MyPick 站点。当前站点不会在抽屉中重复显示。
...
搜索弹窗的滚动结构必须让筛选区和歌曲列表共用同一个纵向滚动容器。标题栏和底部计数栏可以留在弹窗框架内，但展开后的 Filters 面板不能再单独设置纵向 `max-height` 或 `overflow-y-auto`。这样用户在移动端选择筛选项后，继续向下滑动歌单时，已展开的 Filters 会随列表一起向上滑出视口，不会长期遮挡大部分歌曲结果。
...
左上角 `Other Picks` 抽屉只显示当前项目以外的两个姐妹站点，链接目标来自项目配置中的 `siteUrl`，并可通过按钮、背景或 Escape 关闭。
